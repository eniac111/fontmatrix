...
Node::ListItem::ListItem(Node *N, double D)
    : n(N)
    , distance(D)
{
        ...
        if (bIndex < lyt->breakList.count()) {
            ...
            if (di >= lyt->lineWidth(deep)) {
                ...
                /// AT CLOSEST BREAK (after)
                {
                    ...
                    if (compressValue <= lyt->FM_LAYOUT_MAX_COMPRESSION) {
                        // 						qDebug()<<"["<<cIdx<<","<<fit<<"]("<< lyt->sepCount(cIdx, fit ,lyt->theString) <<") spaceWidth("<<spaceWidth<<")
                        // disF("<<disF<<") compressValue("<<compressValue<<")";
...
void FMLayout::doLines()
{
    ...
    for (int lIdx(0); lIdx < maxIndex; ++lIdx) {
        ...
        /// * =
            ...
            QString dbh;
            for (int h(0); h < curHyph.count(); ++h) {
                dbh += "[" + QString(QChar(curHyph[h].lChar)) + "]";
            }
            // 			qDebug() <<dgS<<"="<<dbh;
        ...
        if (!deviceIndy) {
            for (int i = 0; i < refGlyph.count(); ++i) {
                ...
                if (contextIsMainThread) {
                    ...
                    //					qDebug()<<refGlyph[i].glyph<<pen.y() << ( refGlyph[i].yoffset * pixelAdjustY ) << mgi->metaData ( GLYPH_DATA_BITMAPTOP
                    //).toDouble();
...
double FMLayout::distance(int start, int end, const GlyphList &gl, bool strip)
{
    ...
    if (verticalLayout) {
        if (!hasHyph && !gList.at(EXend).isBreak) {
            ...
            int bp(end);
            while (QChar(gList.at(bp).lChar).category() != QChar::Separator_Space && bp > start)
                --bp;
            ++bp;
            ...
            int bpE(end);
            while (QChar(gList.at(bpE).lChar).category() != QChar::Separator_Space && bpE > start)
                --bpE;
            ++bpE;
